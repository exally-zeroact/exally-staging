...
T('★日ごとの表でなければ 帯を出さない（給料明細で 出ていた）★', () => {
  /* ★実測 2026-08-28★＝司さんの本の 給料1〜8 で 帯が出て、
     ★「1月分」「合計」「白石　正人　殿」が 人として 並んだ★。
     ⇒ ★別々の日が 15日（半月）に満たない表では 出さない★（実測：給料1〜8 は 1〜2日）。 */
  const d = { '0,0': { v: '給料明細' }, '1,0': { v: '1月分' }, '1,1': { v: '合計' }, '1,2': { v: '白石　正人　殿' } };
  d['2,0'] = { v: 46043 }; d['2,1'] = { v: 56647 }; d['2,2'] = { v: 47.75 };
  d['3,0'] = { v: 46044 }; d['3,1'] = { v: 51716 }; d['3,2'] = { v: 44 };
  const 姿 = Kirikae.見る({ name: '給料1', data: d });
  ok(!姿.つかえる, '★日ごとの表でないのに 出している★');
  /* 断る理由は 2つ在る（日付の列が無い／日ごとの表でない）。★どちらでも 出さない事★が 決まり。 */
  ok(姿.なぜ, '★理由を 言っていない★');
  /* ★日付の列は 在るが 日が少ない★形でも 出さない（3日だけの台帳） */
  const d2 = { '0,0': { v: '題' }, '1,0': { v: '日付' }, '1,1': { v: '白石' }, '1,2': { v: '長野' } };
  for (let r = 2; r < 5; r++) { d2[r + ',0'] = { v: 46023 + r }; d2[r + ',1'] = { v: r }; d2[r + ',2'] = { v: r }; }
  const 姿2 = Kirikae.見る({ name: 'あ', data: d2 });
  ok(!姿2.つかえる, '★3日だけの表で 出している★');
  ok(姿2.なぜ.indexOf('日ごとの表では ありません') === 0, '★理由が 違う：' + 姿2.なぜ);
});
T('★金額の列を 日付の列にしない（散らばりが 広い）★', () => {
  /* 56,647円 は 通し番号だと 2055年。7,475円 は 1920年。★40年 散らばる＝日付ではない★ */
  const d = { '0,0': { v: '明細' }, '1,0': { v: '金額' }, '1,1': { v: '時間' } };
  for (let r = 2; r < 30; r++) { d[r + ',0'] = { v: 7000 + r * 2000 }; d[r + ',1'] = { v: r }; }
  const 姿 = Kirikae.見る({ name: 'あ', data: d });
  ok(!姿.つかえる, '★金額の列を 日付にしている★');
});
T('★在る月だけ 出す（空振りの月を 選ばせない）★', () => {
  const 姿 = Kirikae.見る(表を作る(['白石正人'], 46023, 31));
  ok(姿.つかえる, '★1か月ぶん（31日）の表で 出ない★');
...
T('★今 何を見ているかを 1行 出す★', () => {
  ...
  eq(Kirikae.見る所を決める(sh, 姿, { 月: '2026-01', 始まりの日: 1 }).いま, '全部／1月分（1/1〜1/31）／31行');
  eq(Kirikae.戻す().いま, '全部／全期間');
});
...
T('★月を選んだら 当たらない行は 全部 隠す（Excelのフィルタと 同じ）★', () => {
  /* ★2026-08-28 実物の絵を見て 変えた★
     前は「日付でない行（合計の行など）は そのまま出す」にしていたが、
     実物の給料表は ★途中に 表の題・見出し・合計が 何度も 入る★ ので、
     2/21〜3/20 を選んでも ★別の月の合計や 空行が 混ざって出た★（絵で見えた）。
     ⇒ Excelのフィルタと同じ＝★見出しより下は 当たらない行を 全部 隠す★。 */
  const sh = 表を作る(['白石正人'], 46023, 40);
  sh.data['42,0'] = { v: '' }; sh.data['42,1'] = { v: 999999 };
  const 出 = Kirikae.見る所を決める(sh, Kirikae.見る(sh), { 月: '2026-01', 始まりの日: 1 });
  ok(出.隠す行[42], '★期間の外の行が 残っている（別の月の合計が 混ざる）★');
  ok(!出.隠す行[0] && !出.隠す行[1], '★表の題と 見出しの行まで 隠している★');
});
T('★月を選んでいない時は 1行も 隠さない★', () => {
  const sh = 表を作る(['白石正人', '長野孝'], 46023, 40);
  sh.data['42,0'] = { v: '' }; sh.data['42,1'] = { v: 999999 };
  const 出 = Kirikae.見る所を決める(sh, Kirikae.見る(sh), { 人: '白石正人' });
  eq(Object.keys(出.隠す行).length, 0, '★人だけ選んだのに 行を 隠している★');
});
...
/* ══ ④画面の作り（本物の字を読む） ══ */
T('★「人」と 決めつけない言葉にする（2026-08-28 指示役が決めた）★', () => {
  /* ★実データでは 売上表・距離の見出しが 現金／請求書／PayPay＝人ではない★
     ⇒ ★機械に「人かどうか」を 当てさせない★（当て間違えると 客に嘘を見せる）。 */
  const b = 注記を外す(book, { html: true });
  ok(b.indexOf('<span class="kk-lb">どれを見る</span>') > 0, '★「どれを見る」に なっていない★');
  ok(b.indexOf('見る人') < 0, '★「見る人」が 残っている★');
  ok(b.indexOf('<option value="">全部</option>') > 0, '★選ぶ所の 既定が「全部」でない★');
  ok(b.indexOf('全員') < 0, '★「全員」が 残っている★');
  ok(b.indexOf('<span class="kk-lb">見る月</span>') > 0, '★「見る月」は そのまま のはず★');
  eq(Kirikae.戻す().いま, '全部／全期間');
});
T('★切り替えと 切り出しを 言葉で 分けている★', () => {
  const b = 注記を外す(book, { html: true });
  ok(b.indexOf('全部／全期間に戻す') > 0, '★戻す口が 無い★');
...
if (SELF) {
  ...
  const BREAKS = [
    ...
    ['lib/kirikae.js', '★期間の外を 隠さない★',
      (s) => s.replace('if (!ymd || !KI.期間の中か(ymd, 期間)) { 隠す行[r] = true; continue; }', '')],
    ['lib/kirikae.js', '★見出しの行から 隠し始める（何の表か 分からなくなる）★',
      (s) => s.replace('for (var r = 姿.見出しの行 + 1; r <= 姿.最大行; r++) {', 'for (var r = 0; r <= 姿.最大行; r++) {')],
    ['lib/kirikae.js', '★見出しの行も 隠す（何の表か 分からなくなる）★',
      (s) => s.replace('for (var r = 姿.見出しの行 + 1; r <= 姿.最大行; r++) {', 'for (var r = 姿.見出しの行; r <= 姿.最大行; r++) {')],
    ...
    ['lib/kirikae.js', '★今 何を見ているかを 言わない★',
      (s) => s.replace("いま: (人の名 || '全部') + '／' + (期間 ? 期間.言い方 : '全期間') + '／' + 見える行 + '行',", "いま: '',")],
    ['lib/kirikae.js', '★戻しても 隠れが 残る★',
      (s) => s.replace("return { 隠す行: {}, 隠す列: {}, いま: '全部／全期間' };", "return { 隠す行: { 3: true }, 隠す列: {}, いま: '全部／全期間' };")],
    ...
    ['book.html', '★戻す口を 消す★', (s) => s.replace('全部／全期間に戻す', '戻す')],
    ...
    ['lib/kirikae.js', '★期間を選んでも 当たらない行を 残す（別の月の合計が 混ざる）★',
      (s2) => s2.replace('if (!ymd || !KI.期間の中か(ymd, 期間)) { 隠す行[r] = true; continue; }', 'if (!ymd) continue;')],
    ['lib/kirikae.js', '★月を選んでいないのに 隠す★',
      (s2) => s2.replace('if (!期間) continue;                       /* 月を選んでいない時は 1行も 隠さない */', '')],
    ['lib/kirikae.js', '★日ごとの表でなくても 帯を出す★',
      (s2) => s2.replace('if (別々の日 < 決まり.日が何日以上) {', 'if (false) {')],
    ['book.html', '★「見る人」に 戻す（人だと 決めつける）★',
      (s2) => s2.replace('<span class="kk-lb">どれを見る</span>', '<span class="kk-lb">見る人</span>')],
    ['book.html', '★既定を「全員」に 戻す★',
      (s2) => s2.replace('<option value="">全部</option>', '<option value="">全員</option>')],
    ['lib/kirikae.js', '★今 見ている物の1行を「全員」に 戻す★',
      (s2) => s2.replace("(人の名 || '全部')", "(人の名 || '全員')")],
  ];
  let red = 0;
