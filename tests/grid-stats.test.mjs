...
T('★Infinity / NaN を 合計に入れない（保険。実測では起きない）★', () => {
  /* ★2026-08-21 直した間違い★
     前は「=1/0 は Infinity を返す」と書いていたが、それは ★jsdom の途中経過★ だった。
     ★実ブラウザと実配信では #DIV/0!★（指示役の実測と一致）。
     ⇒ ★計算の答えを jsdom で測って 外に出さない★。ここは ★純関数に 直接 Infinity を渡して★ 数える。 */
  const s = win.GridStats.summarize([{ v: 10 }, { v: '', f: '=1/0', d: 'Infinity' }, { v: 20 }]);
  eq(s.合計, null, '★合計に Infinity を出している★');
  eq(s.計算できていない, 1, '数えていない');
  ok(s.出せない理由.indexOf('計算できていない値') >= 0, '理由を出していない：' + s.出せない理由);
});
