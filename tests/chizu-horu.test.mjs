...
/* ══ ★見出し（2026-08-27 指示役の指摘：使い物になっていない）★ ══ */
T('★Excelが勝手に付けた名前（列1・列2）は 出さない★', () => {
  const m = Chizu.作る([{ name: '月別', data: { '0,0': { v: '列1' }, '0,1': { v: '売上合計' }, '0,2': { v: 'バ1～10' } } }], 網, null);
  ok(m.字.indexOf('列1') < 0, '★「列1」を 出している（中身を1つも表していない）★');
  ok(m.字.indexOf('B=売上合計') > 0, '★本当の見出しまで 消している★');
});
T('★数は 見出しに出さない（「2」「3」は 値であって 見出しではない）★', () => {
  const m = Chizu.作る([{ name: '計算', data: { '0,0': { v: '日付' }, '0,4': { v: 2 }, '0,5': { v: '時数2' } } }], 網, null);
  ok(m.字.indexOf('E=2') < 0, '★値を 見出しとして 出している★');
  ok(m.字.indexOf('F=時数2') > 0);
});
T('★1行目が見出しでない時は 2〜5行目も見て、何行目かを 書く★', () => {
  const m = Chizu.作る([{ name: '給料1', data: {
    '0,0': { v: '給料明細' },
    '1,0': { v: '1月分' }, '1,2': { v: '~1/31' }, '1,3': { v: '合計' },
    '3,0': { v: '日付' },
  } }], 網, null);
  ok(m.字.indexOf('見出し(2行目): A=1月分') > 0, '★2行目を 見ていない／何行目か 書いていない★：' + m.字.slice(m.字.indexOf('- 給料1'), m.字.indexOf('- 給料1') + 110));
});
T('★1行目が見出しの時は 行番号を 書かない（無駄に長くしない）★', () => {
  const m = Chizu.作る([{ name: 'あ', data: { '0,0': { v: '日付' }, '0,1': { v: '売上' } } }], 網, null);
  ok(m.字.indexOf('見出し: A=日付') > 0, '★1行目なのに 行番号を 書いている★');
});
T('★1つも取れない時は はっきり言う（黙って空にしない）★', () => {
  const m = Chizu.作る([{ name: '数だけ', data: { '0,0': { v: 1 }, '1,0': { v: 2 } } }], 網, null);
  ok(m.字.indexOf('（見出しが取れません）') > 0, '★黙って 空にしている★');
});
T('★同じ見出しのシートが並んだら 目印で 見分けられるようにする★', () => {
  const 作 = (n, 月) => ({ name: n, data: { '0,0': { v: '給料明細' }, '2,0': { v: 月 } } });
  const m = Chizu.作る([作('給料1', '1月分'), 作('給料2', '2月分')], 網, null);
  ok(m.字.indexOf('目印: 1月分') > 0 && m.字.indexOf('目印: 2月分') > 0,
    '★8シートとも同じ見出しで 見分けられない★：' + m.字.slice(m.字.indexOf('- 給料1'), m.字.indexOf('- 給料1') + 160));
});
T('★見出しが違うシートには 目印を足さない（無駄に長くしない）★', () => {
  const m = Chizu.作る([{ name: 'あ', data: { '0,0': { v: '日付' } } }, { name: 'い', data: { '0,0': { v: '金額' } } }], 網, null);
  ok(m.字.indexOf('目印:') < 0, '★要らない目印を 足している★');
});
...
if (!fs.existsSync(実物)) {
  ...
  T('★実物の見出しが 使い物になる（列◯が0件・給料1〜8が見分けられる）★', () => {
    /* ★2026-08-27 指示役の指摘★
       前は … 計算「B=列2 / E=2 / G=3」／月別「A=列1」／給料1〜8 は8つとも「A=給料明細」だけ */
    ok(!/列\d+/.test(m.字), '★Excelが勝手に付けた名前（列◯）が 出ている★');
    const 給料 = [];
    for (const l of m.字.split(String.fromCharCode(10))) if (/^- 給料[1-8]（/.test(l)) 給料.push(l.slice(l.indexOf('見出し')));
    eq(給料.length, 8, '給料1〜8が 8枚 無い');
    eq(new Set(給料).size, 8, '★8シートが 見分けられない（同じ見出しが並んでいる）★：' + 給料[0]);
    ok(給料[0].indexOf('1月分') > 0 && 給料[7].indexOf('8月分') > 0, '★何月分か 分からない★');
    console.log('       … 列◯ 0件／給料1〜8 が 1月分〜8月分で 見分けられる');
  });
...
if (SELF) {
  ...
  const BREAKS = [
    ...
    ['lib/chizu.js', '★Excelが勝手に付けた名前（列◯）を 出す★',
      (s) => s.replace('if (勝手な名前.test(s)) return false;', '')],
    ['lib/chizu.js', '★数も 見出しに出す★',
      (s) => s.replace('function 言葉らしいか(cell) {', "function 言葉らしいか(cell) { if (cell && cell.v !== undefined && cell.v !== null && cell.v !== '') return true;")],
    ['lib/chizu.js', '★1行目しか 見ない（給料1〜8が 見分けられない）★',
      (s) => s.replace('var 見る = Math.min(見る行数 || 5, 5);', 'var 見る = 1;')],
    ['lib/chizu.js', '★何行目かを 書かない（黙って別の行を出す）★',
      (s) => s.replace("+ ((s.見出しの行 > 0) ? ('(' + (s.見出しの行 + 1) + '行目)') : '')", '')],
    ['lib/chizu.js', '★取れない時に 黙って 空にする★',
      (s) => s.replace("見出し字 = ' 見出し: （見出しが取れません）';", "見出し字 = '';")],
    ['lib/chizu.js', '★同じ見出しでも 目印を足さない★',
      (s) => s.replace('if (数える[key2] > 1) 姿[q2].目印 = 目印を探す(姿[q2]);', '')],
    ['lib/chizu.js', '★見出しを 入れない★', (s) => s.replace("': ' + s.見出し.join(' / ');", "': ';")],
