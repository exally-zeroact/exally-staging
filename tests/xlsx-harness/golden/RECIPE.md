...
## 1. 真値を作った環境（2026-07-31 初回 → 2026-08-10 取り直し）

| 項目 | 値 |
|---|---|
| 製品 | Microsoft 365（`O365HomePremRetail`） |
| バージョン | **16.0.20228.20158**（2026-08-10 時点。初回は 16.0.20228.20124） |
| プラットフォーム | x64 |
...
> **2026-08-10 の取り直し**（TEXT の書式コードのケースを17件足した時）
> Excel が `.20124` → `.20158` に上がっていたので全ケースを取り直した。
> **既存403ケースは値も表示文字（`text`）も1件残らず同一**（`DIFF-2026-08-10.md` に記録）。
> ＝この更新で Excel 側の答えは変わっていない。差分は新設した17ケースだけ。
