...
T('★画面も 既定は「今のファイルの分だけ」（開くたびに 今のファイルへ戻る）★', () => {
  ...
  /* ★司さん 2026-08-26「このタブに全部だしてどうするんど」★
     ＝ブックを開いている時に ★ぜんぶのファイル★を並べたのは 余計だった。
     ⇒ ★切り替えの帯そのものを 出さない★（選ぶ物が無いなら 並べない） */
  ok(book.indexOf('ぜんぶのファイル') < 0, '★切り替えの帯が まだ在る（ごちゃごちゃする）★');
  /* ★数が 食い違わない★＝「1件」と言いながら 種類の帯が「ぜんぶ3」では 客が混乱する */
  ok(/作る\('ぜんぶ', r\.このファイルの件数,/.test(book), '★種類の帯の数が ほかのファイルの分まで 数えている★');
  /* ★同じ名前で 中身が違う物だけは 隠さない★（めったに出ない・出たら大事） */
...
if (SELF) {
  ...
  const BREAKS = [
    ...
    ['book.html', '★どのファイルの分かを 言わない★',
      (s) => s.replace("(_rirekiFile ? ('「' + _rirekiFile + '」の分 ') : '')", "''")],
    ['book.html', '★同じ名前の別物を 隠す★',
      (s) => s.replace('同じ名前で 中身が違うファイルが ', '')],
