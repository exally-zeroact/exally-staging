...
/* ★ここから下は「見つけた事」を消さないための記録テスト。
   発効日は県ごとに違う（令和7年10月3日〜令和8年3月31日に順次）。
   lib の saiteiNendoOf は【全県10月一律】で切り替えるので、
   10月以降に発効する県では、発効前の月に新しい(高い)額を使ってしまう＝誤警告が出る。
   ★直すかどうかは判断待ち。ただし「そういう県が何県あるか」は毎回見えるようにしておく。 */
T('★発効日が10月中でない県の数を可視化（誤警告の可能性がある範囲）', () => {
  ...
  console.log('      → lib の saiteiNendoOf は全県10月一律で切替。発効前の月に新しい額を使う＝誤警告の可能性（判断待ち）');
});
