...
(function (root) {
  ...
  function 見つける(sheets, opt) {
    ...
    /* ★②VBAが要る事を レシピで済ませて 提案する（★VBAは勧めない★）★
       ＝マクロは そのまま残すが ここでは 動かない。
         ★「出来ません」で終わらせない★＝代わりに 何が出来るかを こちらから言う。
       ★数える物ではない★ので 何本は 0（か所の合計に 混ぜない）。 */
    if (opt.マクロ) 提案.push(作る('vba_to_recipe', 0, [], true, 版));
    /* ★多い順★（効く所から出す）／★出しすぎない★ */
  ...
  var 言い方 = {
    ...
    vba_to_recipe: {
      題: 'マクロ（VBA）でしている事は 手順として 覚えられます',
      なに: 'マクロは そのまま残しますが、ここでは 動きません。',
      こう: '毎月やっている事を そのまま話してください。手順として覚えるので、'
        + '次からは 同じ事を AIを使わずに 出来ます。',
    },
    if_nest: {
      題: 'IF の入れ子が 深くなっています',
  ...
  function 知らせの字(結果) {
    ...
    var 合計 = 0, 数える = 0;
    for (var i = 0; i < 出せる.length; i++) {
      if (出せる[i].数える物か) { 合計 += 出せる[i].何本; 数える++; }
    }
    /* ★数える物（式の書き換え）と 数えない物（マクロ）を 混ぜない★
       ＝「2通り・9,381か所」のように マクロを 1か所 と数えると 嘘になる。 */
    var 文 = 数える
      ? ('式を ' + 結果.見た式 + '本 見て、' + 数える + '通り・' + 合計 + 'か所 見つけました。')
      : '';
    var マクロ = false;
    for (var j = 0; j < 出せる.length; j++) if (出せる[j].種類 === 'vba_to_recipe') マクロ = true;
    if (マクロ) 文 += 'マクロ（VBA）でしている事も 手順として 覚えられます。';
    return {
      題: 数える ? 'もっと短く書ける所が あります' : 'こちらで 出来る事が あります',
      本文: 文 + 'AIは使っていません。',
