...
(function (root) {
  ...
  /* ★見出しは「1行目」とは限らない★（2026-08-27 指示役の指摘）
     実物で こうなっていた …
       計算 … 「A=日付 / ★B=列2★ / C=つかさ / D=時数 / ★E=2★ …」
       月別 … 「★A=列1★ / B=売上合計 …」
       給料1〜8 … ★8シートとも「A=給料明細」だけ★＝8つの違いが 分からない
     ⇒ ★「列1」「列2」は Excelが勝手に付けた名前★＝中身を1つも表していない。出さない。
     ⇒ ★1行目から5行目までを見て いちばん「言葉が多い行」を 見出しにする★
        （給料1は 2行目に「1月分」＝★8つを 見分けられる★）
     ⇒ ★1行目でない時は 何行目かを 書く★（黙って別の行を出さない）
     ⇒ ★1つも取れない時は「（見出しが取れません）」★（黙って空にしない） */
  var 勝手な名前 = /^(列\s*\d+|Column\s*\d+)$/i;

  function 言葉らしいか(cell) {
    if (!cell) return false;
    var v = cell.v;
    if (v === undefined || v === null || v === '') return false;
    if (typeof v === 'number') return false;            /* 数（日付の通し番号も 数） */
    var s = String(v).trim();
    if (s === '') return false;
    if (勝手な名前.test(s)) return false;               /* ★Excelが勝手に付けた名前★ */
    if (/^[\d,.\-¥￥%]+$/.test(s)) return false;         /* 数の形の字 */
    return true;
  }

  /** ★見出しの行を 探す★（1〜5行目のうち 言葉がいちばん多い行） */
  function 見出しの行を探す(data, 最大列, 見る行数) {
    var 一番 = -1, 一番の数 = 0;
    var 見る = Math.min(見る行数 || 5, 5);
    for (var r = 0; r < 見る; r++) {
      var 数 = 0;
      for (var c = 0; c <= 最大列; c++) if (言葉らしいか(data[r + ',' + c])) 数++;
      if (数 > 一番の数) { 一番の数 = 数; 一番 = r; }
    }
    return { 行: 一番, 数: 一番の数 };
  }
  ...
  function シートの姿(sheet, 見出しの数) {
    ...
    var 見つけた = 見出しの行を探す(data, 最大列);
    var 見出し = [];
    if (見つけた.行 >= 0) {
      for (var ci = 0; ci <= 最大列 && 見出し.length < (見出しの数 || 12); ci++) {
        var cell = data[見つけた.行 + ',' + ci];
        if (!言葉らしいか(cell)) continue;              /* ★数も 勝手な名前も 出さない★ */
        見出し.push(列の字(ci) + '=' + String(cell.v).trim());
      }
    }
    ...
    return {
      ...
      式: 式, 見出し: 見出し,
      見出しの行: 見つけた.行,                 /* 0 なら1行目・-1 なら 取れなかった */
      data: data, 最大行: 最大行, 最大列: 最大列,
    };
  }

  /** ★同じ見出しのシートが並んだ時に 見分けるための目印★
   *  （実物の 給料1〜8 は 8つとも「給料明細」だけだった＝どれが誰の月か 分からない） */
  function 目印を探す(姿) {
    var data = 姿.data || {};
    var 出 = [];
    for (var r = 0; r <= Math.min(姿.最大行, 12) && 出.length < 2; r++) {
      if (r === 姿.見出しの行) continue;
      for (var c = 0; c <= Math.min(姿.最大列, 8) && 出.length < 2; c++) {
        var cell = data[r + ',' + c];
        if (!言葉らしいか(cell)) continue;
        var s = String(cell.v).trim();
        if (s.length > 20) continue;
        if (出.indexOf(s) >= 0) continue;
        出.push(s);
      }
    }
    return 出;
  ...
  function 作る(sheets, 網, 診断, opt) {
    ...
    /* ★同じ見出しのシートが並んだら 見分けられない★（実物の 給料1〜8）
       ⇒ ★同じ物が2枚以上 在る時だけ 目印を足す★（無駄に長くしない） */
    var 数える = {};
    for (var q = 0; q < 姿.length; q++) {
      var key = 姿[q].見出し.join('|');
      数える[key] = (数える[key] || 0) + 1;
    }
    for (var q2 = 0; q2 < 姿.length; q2++) {
      var key2 = 姿[q2].見出し.join('|');
      if (数える[key2] > 1) 姿[q2].目印 = 目印を探す(姿[q2]);
    }
    ...
    for (var i = 0; i < 姿.length; i++) {
      ...
      /* ★見出しは 1行目とは限らない★＝★何行目かを 書く★（黙って別の行を出さない）
         ★1つも取れない時は はっきり言う★（黙って空にしない） */
      var 見出し字;
      if (!s.見出し.length) {
        見出し字 = ' 見出し: （見出しが取れません）';
      } else {
        見出し字 = ' 見出し'
          + ((s.見出しの行 > 0) ? ('(' + (s.見出しの行 + 1) + '行目)') : '')
          + ': ' + s.見出し.join(' / ');
      }
      if (s.目印 && s.目印.length) 見出し字 += ' ／ 目印: ' + s.目印.join(' / ');
      行.push('- ' + s.名 + '（' + 大きさ + '・式' + s.式 + '本）' + 見出し字);
    }
    行.push('');
  ...
  /* ★見出しの見つけ方は ここ1か所だけ★（2026-08-27）
     ＝レシピ側が 1行目を そのまま見出しにしていて、実物の給料表では
       「46023」(日付の通し番号) が 見出しに入っていた。★来月には 変わる字★を
       覚えの鍵にすると、翌月 当たらなくなる。⇒ 同じ道具を 使わせる。 */
  var api = { 作る: 作る, シートの姿: シートの姿, 参照のまとめ: 参照のまとめ, 列の字: 列の字, 既定の上限: 既定の上限,
    見出しの行を探す: 見出しの行を探す, 言葉らしいか: 言葉らしいか };
