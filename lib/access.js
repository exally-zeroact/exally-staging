 ...
 *  ★現段階は「使える or 停止」のシンプルなON/OFFのみ。
 *    無料期間(期限)や有料/無料の細分は、プランを決める時に実装する(expires_at列はDBに予約済・未参照)。★
 *  返り値: { ok:boolean, reason:'new'|'trial'|'paid'|'free'|'disabled' }
 *
 * ★★ドリフト注意★★
 *   同じ判定が Kyually(payslip-app/lib/access.js)にもある。別リポジトリで import できないため
 *   実体は2つあるが、**tests/access-drift.test.js が全plan値で両者の判定一致を検証する**。
 *   ここを変えるなら Kyually 側も同時に変える(片方だけ変えると「停止したはずのアカウントが使える」事故)。
 */
