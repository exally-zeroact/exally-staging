 ...
 *
 *  ★測っていない事（勝手に決めない・報告に出す）★
 *    ・★すぐ上 と すぐ左 の両方に数が在る時 どちらを採るか★ は測れていない。
 *      上の3通りと矛盾しない「①上を先に見る」で作ってある。
 *      ⇒ 実Excelで1回 押して確かめてもらう（F4の回る順と同じ扱い）。
...
(function (root) {
  ...
  function suggest(get, r, c) {
    ...
    if (c > 0 && isNum(get(r, c - 1))) {
      var left = c - 1;
      while (left > 0 && isNum(get(r, left - 1))) left--;
      var a2 = colName(left) + (r + 1), b2 = colName(c - 1) + (r + 1);
      return { formula: '=SUM(' + (a2 === b2 ? a2 : a2 + ':' + b2) + ')', dir: 'left', from: a2, to: b2 };
    }
    /* ③ ★分かりません★（勝手に広げない） */
    return null;
  }
