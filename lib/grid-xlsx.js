...
(function (root, factory) {
  if (typeof module === 'object' && module.exports) module.exports = factory(require('./typed-value.js'));
  else root.GridXlsx = factory(root.TypedValue);
}(typeof self !== 'undefined' ? self : this, function (TypedValue) {
  'use strict';
  ...
  function asValue(raw) {
    ...
    /* ★桁区切り（1,234）と全角（１，２３４）は数にする（E3）★
       ここを book.html の toHFVal とズラすと、★画面の合計と 落としたファイルの合計が食い違う★。
       規則は lib/typed-value.js の1本だけ＝両方がそこを呼ぶ。 */
    var tv = TypedValue.parseTyped(s);
    if (tv) return tv.num;
    if (s.trim() !== '' && !isNaN(s)) return parseFloat(s);
    return s;
  }
  /* 桁区切りで打った物に、Excelが当てるのと同じ表示形式を返す（そうでなければ null） */
  function typedNumFmt(raw) {
    if (typeof raw !== 'string') return null;
    var tv = TypedValue.parseTyped(raw);
    return (tv && tv.numFmt) ? tv.numFmt : null;
  }
    ...
    return {
      sheets: (sheets || []).map(function (sh) {
        ...
        Object.keys(sh.data || {}).forEach(function (key) {
          ...
          /* ★桁区切りで打った物も同じ（1,234 を数で書くので、書式が無いと相手の画面から桁区切りが消える）★ */
          else if (!isF && typedNumFmt(cell.v !== undefined && cell.v !== '' ? cell.v : cell.d)) {
            out.z = typedNumFmt(cell.v !== undefined && cell.v !== '' ? cell.v : cell.d);
          }
          cells[addr(r, c)] = out;
