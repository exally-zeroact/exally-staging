...
const SHAKAIHOKEN_HYO = {
  ...
  KENKO_RITSU: {
    ...
    shizuoka:  { name: '静岡県', total: 0.098, jugyoin: 0.049 },
  ...
  // 中央(Supabase statutory kind=shakaihoken)の値で上書き。健保はtotal上書き+従業員=total/2再計算。
  // 47県そろってる時だけ上書き=壊れた部分データで既存を潰さない。取れない/不正なら何もしない=フォールバック。
  hydrate: function(data) {
    if (!data || typeof data !== 'object') return;
    var kt = data.kenko_total;
    if (kt && typeof kt === 'object' && Object.keys(kt).length >= 40) {
      var self = this;
      Object.keys(kt).forEach(function(p) { if (self.KENKO_RITSU[p]) { self.KENKO_RITSU[p].total = kt[p]; self.KENKO_RITSU[p].jugyoin = kt[p] / 2; } });
    }
    if (data.kaigo_total != null) { this.KAIGO_RITSU_TOTAL = data.kaigo_total; this.KAIGO_RITSU_JUGYOIN = data.kaigo_total / 2; }
    if (data.kosei_total != null) { this.KOSEI_NENKIN_RITSU_TOTAL = data.kosei_total; this.KOSEI_NENKIN_RITSU_JUGYOIN = data.kosei_total / 2; }
  }
